    ...
    with pyczi.open_czi(czifile_scenes) as czidoc:
        ...
        for i in range(0, len(scenes_bounding_rectangle)):
            ...
            with alive_bar(len(scenes_bounding_rectangle),force_tty=True) as bar:
                ...
                print(my_real_roi)
                ch0_downsampled = czidoc.read(roi=my_real_roi, plane={'C': 0}, scene=1, zoom=zoom_factor)
                ch1_downsampled = czidoc.read(roi=my_real_roi, plane={'C': 1}, scene=1, zoom=zoom_factor)
