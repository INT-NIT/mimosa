...
def load_or_freeze_slice_reference_in_yaml(cfg, yaml_path, subject,
    ...
    The total (NumberOfSlices) is stored under the subject's entry as
    ``slice_reference``. On later runs the stored map is reused unchanged (no
    separate slice_reference.json). This lets you fix the slice total of a brain:
    list only the slices you want in the YAML, run once (or with refreeze=True) to
    freeze that total, and every exported slice lands at a contiguous position
    0,1,2,... so the volume is dense (no black gaps) when you convert them all.
    Pass refreeze=True to recompute the total from the current slice list.
    """
    subject = str(subject)
