...
def write_json_sidecar_for_ome_tiff(image_path: Path, meta: dict) -> None:
    """Write the JSON sidecar next to an OME-TIFF.
 
    Example:
        image.ome.tif -> image.json
    """
...
def get_slice_indices_for_czi(cfg: dict, czi_filename: str) -> list[int]:
    """Slice indices for one CZI from metadata.yml, else from the filename."""
...
def build_raw_bids_output_path(
    ...
    """Build the raw BIDS OME-TIFF path.
 
    BIDS microscopy only allows the ``.ome.tif`` extension (NOT ``.ome.tiff``),
    otherwise the validator raises EXTENSION_MISMATCH.
 
    Example:
        sub-Una/ses-01/micr/sub-Una_ses-01_sample-slide01_stain-C0_FLUO.ome.tif
    """
    out_dir = Path(bids_root) / f"sub-{subject}" / f"ses-{session}" / "micr"
...
def rect_to_xywh(rect):
    """Return x, y, w, h from a pylibCZIrw rectangle or a tuple/list."""
...
def iter_scene_rectangles(scenes):
    """Iterate scene rectangles whether scenes is a list or a dict."""
...
def build_scene_positions_metadata(scenes, total_bbox, downsampling_factor: int) -> list[dict]:
    """Original CZI scene boxes and their positions in the output mosaic."""
        ...
        scene_positions.append(
            {
                ...
                "CziBoundingBox": {"X": x, "Y": y, "Width": w, "Height": h, "Units": "pixels"},
                "OutputBoundingBox": {"X": output_x, "Y": output_y, "Width": output_w, "Height": output_h, "Units": "pixels"},
            }
        )
...
def make_sidecar_metadata(
    ...
    PixelSize and PixelSizeUnits are REQUIRED by the BIDS microscopy validator
    (SIDECAR_KEY_REQUIRED). They must be consistent with the OME PhysicalSize
    written in the OME-TIFF (same value, same micrometre unit).
    """
    ...
    return {
        ...
        # --- required by BIDS ---
        "PixelSize": [float(pixel_size_x_um), float(pixel_size_y_um)],
        "PixelSizeUnits": "um",
...
def get_next_slide_label(samples_rows: list[dict], participant_id: str) -> str:
    """Build sample labels slide01, slide02, ..."""
    count = 0
...
def inject_scene_positions_into_ome_xml(existing_xml: str, scenes, total_bbox, downsampling_factor: int) -> str:
    """Add scene positions as a MapAnnotation into the OME-XML from tifffile.
 
    Only adds a StructuredAnnotations block; does not touch SizeX/SizeY/TiffData.
    """
...
def read_zoomed_patch(
    czidoc, channel, bbox_x, bbox_y, bbox_w, bbox_h,
    patch_width_full, patch_height_full, x_idx, y_idx, downsampling_factor,
):
    """Read one CZI patch directly at the reduced resolution (CZI zoom)."""
    source_x = x_idx * patch_width_full
    source_y = y_idx * patch_height_full
 
    patch_width = min(patch_width_full, bbox_w - source_x)
    patch_height = min(patch_height_full, bbox_h - source_y)
    if patch_width <= 0 or patch_height <= 0:
        return None
 
    roi = (int(bbox_x + source_x), int(bbox_y + source_y), int(patch_width), int(patch_height))
 
    patch_res = np.asarray(
        czidoc.read(roi=roi, plane={"C": int(channel)}, zoom=1.0 / float(downsampling_factor))
    ).squeeze()
    if patch_res.ndim == 3:
...
def convert_czi_total_bbox_2_ome_tiff(
    ...
    print("Subject:", subject, "| Session:", session, "| Sample:", sample_label)
    print("SliceIndices:", slice_indices, "| Channels:", channels)
    ...
    pixel_size_x_um = float(native_px_x_um) * float(downsampling_factor)
    pixel_size_y_um = float(native_px_y_um) * float(downsampling_factor)
    print(f"\nOutput pixel size: X={pixel_size_x_um} um  Y={pixel_size_y_um} um")
    ...
    with pyczi.open_czi(str(input_czi)) as czidoc:
        ...
        print("\nTotal bounding rectangle:", (bbox_x, bbox_y, bbox_w, bbox_h))
        ...
        for channel in channels:
            ...
            mosaic_image = np.zeros((mosaic_image_height, mosaic_image_width), dtype=np.uint16)
 
            with alive_bar(total_patches, force_tty=True,
                           title=f"{sample_label} {stain_label} ds{downsampling_factor}x") as bar:
                for x_idx, y_idx in patch_indices:
                    ...
                    if result is None:
                        bar()
                        continue
                    patch_res, output_x, output_y = result
                    output_x_end = min(output_x + patch_res.shape[1], mosaic_image_width)
                    output_y_end = min(output_y + patch_res.shape[0], mosaic_image_height)
                    mosaic_image[output_y:output_y_end, output_x:output_x_end] = \
                        patch_res[: output_y_end - output_y, : output_x_end - output_x]
                    bar()
 
            output_path = build_raw_bids_output_path(
                bids_root, subject, session, sample_label, stain_label,
            )
 
            # Step 1: temp file to get a valid OME-XML from tifffile, with the
            # physical pixel size (BIDS wants the OME PhysicalSize consistent
            # with the JSON PixelSize).
            with tempfile.NamedTemporaryFile(suffix=".ome.tif", delete=False) as tmp:
                tmp_path = tmp.name
            tifffile.imwrite(
                tmp_path, mosaic_image, photometric="minisblack", ome=True,
                metadata={
                    "axes": "YX",
                    "PhysicalSizeX": pixel_size_x_um, "PhysicalSizeXUnit": "µm",
                    "PhysicalSizeY": pixel_size_y_um, "PhysicalSizeYUnit": "µm",
                },
            )
            ...
            # Step 2: inject scene positions into the OME-XML.
            enriched_xml = inject_scene_positions_into_ome_xml(
                xml_str, scenes, bbox, downsampling_factor,
            )
 
            # tifffile stores the ImageDescription as 7-bit ASCII only. The
            # OME-XML contains "µm" (µ is non-ASCII) and would raise
            # "TIFF strings must be 7-bit ASCII". Encode non-ASCII as XML
            # numeric entities (µ -> &#181;); OME readers decode them back.
            enriched_xml = enriched_xml.encode("ascii", "xmlcharrefreplace").decode("ascii")
 
            # Step 3: write the final BigTIFF, lossless zlib, with the enriched XML.
            ...
            with tifffile.TiffFile(str(output_path)) as tf:
                print("is_ome:", tf.is_ome, "| is_bigtiff:", tf.is_bigtiff,
                      "| shape:", tf.series[0].shape, "| axes:", tf.series[0].axes)
 
 
def main():
    parser = argparse.ArgumentParser(
        description="Convert CZI files listed in metadata.yml to raw BIDS OME-TIFF (zlib, BigTIFF)."
    )
    ...
    for item in get_subject_entries(cfg):
        ...
        try:
            ...
            bids_info = session.get_bids_info(summary_meta=summary, czi_id=czi_id, section_idx=None)
            subject = bids_info["sub"]
            session_id = bids_info["ses"]
            print("BIDS info:", bids_info)
 
            bm.create_sourcedata_links(str(input_czi), subject, str(bids_root))
 
            participant_id = item["sample_info"].get("participant_id", f"sub-{subject}")
            sample_label = get_next_slide_label(samples_rows, participant_id)
    ...
    for sub, session_dict in sessions_by_sub.items():
        rows = [{"session_id": ses_id, "acq_time": session_dict[ses_id]}
                for ses_id in sorted(session_dict.keys())]
        bmeta.write_subject_sessions_tsv(str(bids_root), sub, rows)
