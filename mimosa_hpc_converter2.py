...
def is_valid_slice_value(value):
    """
    Avoid sample-sectionNone / sample-sectionnan / empty values.
    """
    if value is None:
        return False
    ...
    if value_str in ("", "none", "nan", "null", "na", "n/a"):
        return False
...
def main():
    ...
    print("DEBUG YAML ARG:", args.yaml)
    print("DEBUG YAML USED:", Path(args.yaml).resolve())
    print("DEBUG YAML EXISTS:", Path(args.yaml).exists())
    ...
    print(f"DEBUG number of CZI files found: {len(files_to_process)}")
    ...
    for input_dir, filename in files_to_process:
        ...
        # DEBUG IMPORTANT
        slices_from_yaml = bmeta.get_slices_for_file(cfg, filename)
        print("DEBUG input_dir:", input_dir)
        print("DEBUG full_input_path:", full_input_path)
        print("DEBUG filename:", filename)
        print("DEBUG slices from YAML:", slices_from_yaml)
        ...
        try:
            with MimosaReader(full_input_path) as reader:
                ...
                # slices from YAML
                slices = bmeta.get_slices_for_file(cfg, filename)

                print("DEBUG slices used for samples.tsv:", slices)

                for slice_num in slices:
                    if not is_valid_slice_value(slice_num):
                        print(
                            f"    WARNING: invalid slice value for {filename}: "
                            f"{slice_num}, skipping sample-section"
                        )
                        continue
